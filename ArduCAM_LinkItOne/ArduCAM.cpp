...
/*------------------------------------
	...
	2015/11/10  V3.5.0  by Lee	Add support for LinkIt ONE board.		
--------------------------------------*/
#include "Arduino.h"
#include "ArduCAM.h"
...
//User Wire1 instead of Wire when using Arduino DUE board
...
{ 
#if defined(ARDUINO_MTK_ONE) || defined(ESP8266)
	B_CS = CS;
#else
...
void ArduCAM::OV5642_set_JPEG_size(uint8_t size)
{
	...
	switch(size)
	{
		...
		case OV5642_1280x720:
			wrSensorRegs16_8(ov5642_dvp_fmt_jpeg_xga);
			//wrSensorRegs16_8(ov5642_res_720P);

			break;
		case OV5642_1920x1080:
			wrSensorRegs16_8(ov5642_dvp_fmt_jpeg_uxga);
	...
	switch(sensor_model)
	{
      ...
      wrSensorReg8_16(0xC8, 0x0005); //SEQ_CMD
		...
		case OV5642:
		{
			...
			//wrSensorRegs16_8(ov5642_af_firmware);
			delay(100);
			if(m_fmt == JPEG)
			{
				//wrSensorRegs16_8(ov5642_dvp_fmt_global_init); 
